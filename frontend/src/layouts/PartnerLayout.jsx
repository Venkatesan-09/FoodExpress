...
import {
  HomeIcon, MapPinIcon, TruckIcon, CurrencyRupeeIcon,
  UserCircleIcon, ArrowRightOnRectangleIcon, Bars3Icon
} from '@heroicons/react/24/outline'
import { useState } from 'react'

const NAV = [
  { to: '/partner/dashboard',         label: 'Dashboard',        Icon: HomeIcon },
  { to: '/partner/orders/available',  label: 'Available Orders', Icon: MapPinIcon },
  { to: '/partner/orders/active',     label: 'Active Delivery',  Icon: TruckIcon },
  { to: '/partner/earnings',          label: 'Earnings',         Icon: CurrencyRupeeIcon },
]
  ...
  const SidebarContent = () => (
    <>
      <div className="p-6 border-b border-stone-100">
        <Link to="/partner/dashboard" className="flex items-center gap-2 font-display font-bold text-xl">
          <span className="text-2xl">🛵</span>
          <div>
            <div className="gradient-brand">FoodExpress</div>
            <div className="text-xs text-text-tertiary font-sans font-normal">Delivery Partner</div>
          </div>
        </Link>
      </div>
      ...
      <div className="p-4 border-t border-stone-100">
        <div className="flex items-center gap-3 px-2 mb-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-text-tertiary">{user?.vehicleType}</p>
          </div>
        </div>
    ...
    <div className="min-h-screen flex bg-surface-raised">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        <header className="lg:hidden sticky top-0 z-sticky glass border-b border-stone-100 flex items-center gap-3 px-4 h-14">
          ...
          <span className="font-display font-bold gradient-brand">FoodExpress Partner</span>
        </header>
